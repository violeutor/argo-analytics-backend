...
async def get_company_detail(name: str, background_tasks: BackgroundTasks, isin: str | None = None, ticker: str | None = None, exchange: str | None = None, composite_figi: str | None = None) -> CompanyDetailResponse:
    ...
    if company_id and not _market_data_valid:
        async def _market_enrichment_bg():
            try:
                # Category-Gate (Race-Fix S44): ohne aufgelöste Kategorie macht
                # Market-Enrichment keinen Sinn — MD-B01 würde mit leerem Sektor
                # DDG verbrennen, SAM/Competition wären ohne Kontext wertlos.
                # Wir schreiben dann NICHTS (kein sam_usd_bn) → _market_data_fresh
                # bleibt False → nächster Load re-triggert mit aufgelöster Kategorie.
                if not (company.get("category") or "").strip():
                    logger.info(
                        "Market enrichment skip für %s — Kategorie noch nicht aufgelöst "
                        "(Race-Guard). Re-trigger beim nächsten Load.", company_name,
                    )
                    return
